...
describe("#newFullFlow Smoke tests for Continuous Integration", () => {
    let page
    let v3PMpage
    let loginPage
    let userManPage
    ...
    describe("Smoke tests for Continuous Integration", () => {
        ...
        describe(" Languages Support", () => {
            ...
            it("User change language from English to Greek", async () => {
                const selectedLang = await configurationGeneralPage.changeLang("Ελληνικά (Greek)")
                loginPage = new LoginPage(page)
                userManPage = new UserManPage(page)
                resourcesPage = new ResourcesPage(page)
                v3PMpage = new V3ProjectManagementPage(page)
                configurationGeneralPage = new ConfigurationPage(page)
                v3ReviewAndApprovePage = new V3ReviewAndApprovePage(page)
                const isSearchField = await isElementExistsbyXPath(page, v3ReviewAndApprovePage.searchFieldLM)
                assert.equal(selectedLang, "Ελληνικά (Greek)", `selected language is not Greek`)
                assert.isTrue(isSearchField, `search field in greek was not found ${v3ReviewAndApprovePage.searchFieldLM}`)
            })
        })
